...
if not st.session_state.zalogowany:
    ...
    # =========================================================================
    # ZAKŁADKA: NOWE ZAMÓWIENIE (Z CAŁYM ARSENAŁEM ŻARTÓW)
    # =========================================================================
    if menu == "📝 Nowe Zamówienie":
        ...
        with st.container(border=True):
            ...
            if st.toggle("📷 Włącz aparat, aby dodać zdjęcie"):
                zdjecie = st.camera_input("Zrób zdjęcie części")
            
            st.divider()
            col_wa, _ = st.columns([1, 1])
            powiadom_admina = col_wa.selectbox("📲 Powiadom admina (WhatsApp)", opcje_adminow)
            ...
            if st.button("WYŚLIJ ZAMÓWIENIE", type="primary", use_container_width=True):
                if pozycja and ilosc:
                    ...
                    if zdjecie is not None:
                        ...
                        supabase.storage.from_("zdjecia_zamowien").upload(path=nazwa_pliku, file=zdjecie.getvalue(), file_options={"content-type": "image/jpeg"})
                    ...
                    st.balloons(); st.success("✅ Zamówienie wysłane!")
                    if powiadom_admina != "-- Nie wysyłaj --":
                        sn = admin_phones[powiadom_admina]
                        cn = "".join(c for c in sn if c.isdigit())
                        tresc = f"Nowe zamówienie od {st.session_state.uzytkownik}: {pozycja} ({ilosc}). Projekt: {projekt}"
                        st.link_button("📲 Wyślij WhatsApp", f"https://wa.me/{cn}?text={urllib.parse.quote(tresc)}", use_container_width=True)
                        st.stop()
                    else:
    ...
    # --- POZOSTAŁE ZAKŁADKI BEZ ZMIAN W LOGICE ---
        ...
        if not res.data: st.success("Wszystko zrobione!")
        else:
            for r in res.data:
                with st.container(border=True):
                    ...
                    l_stat = ["Oczekujące", "Zamówione", "Niedostępne", "Zamiennik", "Zrealizowane"]
                    n_stat = col_stat.selectbox("Status", l_stat, index=l_stat.index(r['status']), key=f"s_{r['id']}")
        ...
        if res.data:
            ...
            st.subheader("TOP materiały")
            st.bar_chart(df['pozycja'].value_counts().head(10))
        ...
        st.title("📖 Pomoc")
        st.write("Skrócona pomoc: Wypełnij formularz, zrób fotkę jeśli trzeba, wybierz projekt i wyślij. Powiadom admina przez WhatsApp.")
